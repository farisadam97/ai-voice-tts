...
for label, text in tests:
    out = f"G:/Coding/Learn/vtuber-tts/output/response_audio/chatterbox_orig_{label.lower()}.wav"
    t0 = time.time()
    wav = model.generate(text, audio_prompt_path=ref_audio, exaggeration=0.7, cfg_weight=0.3)
    t1 = time.time()
